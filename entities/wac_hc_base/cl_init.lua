...
function ENT:Think()
	...
	if !self:GetNWBool("locked") then
		...
		local volume = tonumber(LocalPlayer():GetInfo("wac_cl_air_volume")) * 0.18
...
function ENT:DrawPilotHud()
	...
	surface.DrawText("FLR")
	surface.SetTextPos(-10,-24)
	local currentFlares = self:GetNWInt("Flares",0) or 0
	surface.DrawText(currentFlares)
	cam.End3D2D()
end
...
function ENT:Draw()
	...
	if self.engineRpm > 0.2 and self.SmokePos then
		...
		if IsValid(self.Emitter) and (!self.lastHeatDrawn or self.lastHeatDrawn < CurTime()-0.1) then
